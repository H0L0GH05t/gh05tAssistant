    ...
    try:
        ...
        for page in document.get_pages():
            ...
            if cropbox:
                ...
                # get extra margins to add
                extra_margin_bottom = float(trimbox[2]) - float(page.mediabox[2])
                # extra_margin_bottom = float(trimbox[0])
                extra_margin_left = float(trimbox[3]) - float(page.mediabox[3])
                # extra_margin_left = float(trimbox[1])
            else:
                                ...
                                if '.' in word or '@' in word or ':' in word:
                                    ...
                                    if url:
                                        ...
                                        if first_match and last_match:
                                            ...
                                            try:
                                                x0 = (lt_text_obj.bbox[0] + extra_margin_bottom)
                                                x1 = (lt_text_obj.bbox[2] + extra_margin_left)
                                                y0 = (lt_text_obj.bbox[1] + extra_margin_bottom)
                                                y1 = (lt_text_obj.bbox[3] + extra_margin_left)
...
if auto_generate_links:
    if pdfminer_available and report_lab_available:
        ...
        get_page_info = 'pdftk "%s" dump_data_utf8' % pdf_path
        ...
        for line in page_info_line:
            if "PageMediaTrimRect" in line:
                trimbox = line.replace('PageMediaTrimRect: ', '').split()
            if "PageMediaCropRect" in line:
                cropbox = line.replace('PageMediaCropRect: ', '').split()
            if "PageMediaRect" in line:
                mediabox = line.replace('PageMediaRect: ', '').split()

        print("Got boxes -- trimbox: %s -- mediabox: %s -- cropbox: %s" % (trimbox, mediabox, cropbox))####
        ...
        if link_page != None:
            ...
            if debug_article_links:
                print("debug on, stamping page")
                # merge the link page on top of the original PDF
                stamp_cmd = 'pdftk "%s" background "%s" output "%s"' % (link_page, pdf_path, "background.pdf")
