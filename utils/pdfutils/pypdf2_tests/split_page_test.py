...
if number_of_pages > 0:
    ...
    try:
        ...
        if center_spread_pdf:
            pdf_page_r = input_pdf.getPage(0)
            output_left = PdfFileWriter()
            ...
            original_upperRight = pdf_page.mediaBox.upperRight
            pdf_page.mediaBox.upperRight = (
                pdf_page.mediaBox.getUpperRight_x() / 2,
                pdf_page.mediaBox.getUpperRight_y()
            )
            output_left.addPage(pdf_page)
            outputStream = open(split_path_left, "wb")
            output_left.write(outputStream)
            outputStream.close()

            #Write right half of page
            output_right = PdfFileWriter()
            split_path_right = "%s-right.pdf" % input_path.replace('.pdf', '')
            pdf_page.mediaBox.upperRight = original_upperRight
            pdf_page_r.mediaBox.upperLeft = (
                pdf_page_r.mediaBox.getUpperRight_x() / 2,
                pdf_page_r.mediaBox.getUpperLeft_y()
            )
            output_right.addPage(pdf_page_r)

            outputStream = open(split_path_right, "wb")
            output_right.write(outputStream)
            outputStream.close()
