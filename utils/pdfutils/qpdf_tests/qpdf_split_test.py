...
def check_for_multiple_page_pdf(file_path, file_name):
        try:
            ...
            if file_type == 'pdf':
                ...
                check_pages_cmd = "qpdf %s --show-npages" % file_name
                ...
                if extension.lower() == '.pdf':
                    ...
                    check_pages_cmd = "qpdf %s --show-npages" % pdf_path
                    process = subprocess.Popen(check_pages_cmd, shell=True, stdout=subprocess.PIPE, )
                    output, unused_err = process.communicate()
                    number_of_pages = int(output.decode('utf-8'))
                    print("Pdf has %s pages. Returning..." % number_of_pages)
...
def split_multiple_page_pdf(index, number_of_pages, file_path, original_name, split_path):
        try:
            ...
            if len(parse_rule):
                ...
                if m is not None:
                    ...
                    if page_selector == 1 and page_num1 > 0:
                        page_number = str(page_num1 + index)
                    elif page_selector == 2 and page_num2 > 0:
                        page_number = str(page_num2 + index)
                    elif page_num > 0:
                        page_number = str(page_num + index)
                    elif page_name > 0:
                        page_number = str(int(page_name) + index)
                    else:
                    ...
                    if add_zeros:
                        ...
                        if int(page_number) > 9 and int(page_number) < 99:
                            zeros_to_add = zeros_to_add - 1
                        elif int(page_number) > 99 and int(page_number) < 999:
                            zeros_to_add = zeros_to_add - 2
                        elif int(page_number) > 999 and int(page_number) < 9999:
                            zeros_to_add = zeros_to_add - 3
                        elif int(page_number) > 9999 and int(page_number) < 99999:
                    ...
                    if os.path.exists(new_file_path + ext):
                        new_file_path = new_file_path + '-' + str(index) #safety in case it tries to override a file
                else:
                    ...
                    new_file_path = os.path.join(split_path, base) + '_' + str(index+1)
                    if os.path.exists(new_file_path + ext):
                        new_file_path = new_file_path + '-' + str(index) #safety in case it tries to override a file
            else:
                print('Could not find parsing rule')
                #file_dirname = os.path.dirname(os.path.abspath(file_path))
                new_file_path =  os.path.join(split_path, base) + '_' + str(index)
                if os.path.exists(new_file_path + ext):
                    new_file_path = new_file_path + '-' + str(index) #safety in case it tries to override a file
            
            output_path = new_file_path + ext
            split_cmd = "qpdf %s --pages %s %s -- %s" % (file_path,file_path,str(index+1),output_path)
            print("running cmd: %s" % split_cmd)####
...
if original_file_name.startswith('.'):
    ...
    if number_of_pages > 1:
        ...
        for index in range(number_of_pages):
            new_path = split_multiple_page_pdf(index, number_of_pages, file_path, original_file_name, split_path)
    ...
    for index, file_in_zip in enumerate(archive_file.namelist()):
        ...
        if file_in_zip.endswith('.pdf'):
            ...
            if number_of_pages > 1 and extension.lower() == '.pdf':
                for index in range(number_of_pages):
                    new_path = split_multiple_page_pdf(index, number_of_pages, pdf_path, file_in_zip, extract_path)
                    print("The new pdf path is %s and would move on to processing next."% new_path)
